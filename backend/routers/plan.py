...
from ..schemas.plan import WorkoutPlan
from ..crud import plan as plan_crud
from ..dependencies import get_current_user

router = APIRouter(prefix="/plans", tags=["plans"])

@router.get("/range/{start_date}/{end_date}", response_model=List[WorkoutPlan])
async def read_plans_for_range(
    start_date: date,
    end_date: date,
    current_user: dict = Depends(get_current_user)
):
    """캘린더에 표시할 특정 날짜 범위의 모든 운동 계획을 가져옵니다."""
    user_id = current_user['user_id']
    plans = await plan_crud.get_plans_by_range(user_id, start_date, end_date)
    return plans
